...
if (!$demoLib) {
    exit('Demo library not set in toolkit.ini.');
}
...
if (!$encoding) {
    exit('Encoding not set in toolkit.ini. Example: ISO-8859-1');
}
...
if (!$conn) {
    exit('<BR>Could not connect. Reason: ' . printArray(i5_error()));
}
...
if (!$list) {
    ...
    if ($listItem = i5_objects_list_read($list)) {
        ...
        exit("<BR>Demo library '$demoLib' NOT found. Ending.");
    }
}
...
if ($doPcml) {
   ...
   <program name="YYPLUS" entrypoint="YYPLUS"  path="/QSYS.LIB/' . $demoLib . '.LIB/YYSRVNORES.SRVPGM" >
    ...
    if (!$pgmHandle) {
        ...
        $input = ['START' => '25', 'RESULT' => '0'];
        $output = ['START' => 'START', 'RESULT' => 'RESULT'];
...
if ($doUserSpace) {
    ...
    $usProperties = [I5_NAME => $userSpaceName, I5_LIBNAME => $userSpaceLib, I5_INIT_VALUE => 'Y'];
    ...
    $bigDesc = [
        ['DSName' => 'BIGDS', 'DSParm' => [
            ['Name' => 'P1', 'IO' => I5_INOUT, 'Type' => I5_TYPE_CHAR, 'Length' => 10, 'Count' => 5],
            ['Name' => 'P2C', 'IO' => I5_INOUT, 'Type' => I5_TYPE_LONG, 'Length' => 4],
            ['Name' => 'P2', 'IO' => I5_INOUT, 'Type' => I5_TYPE_CHAR, 'Length' => 1, 'Count' => 2],
            ['DSName' => 'PS', 'Count' => 2, 'DSParm' => [
                ['Name' => 'PS1', 'IO' => I5_INOUT, 'Type' => I5_TYPE_CHAR, 'Length' => 10],
                ['Name' => 'PS2', 'IO' => I5_INOUT, 'Type' => I5_TYPE_CHAR, 'Length' => 10],
                ['Name' => 'PS3', 'IO' => I5_INOUT, 'Type' => I5_TYPE_CHAR, 'Length' => 10],
    ...
    $success = i5_userspace_get($us, ['BIGDS' => 'BIGDS']);
    if (function_exists('i5_output')) {
        extract(i5_output());
    } // i5_output() required if called in a function
...
if ($doDataQueue) {
    ...
    // send
    $key = 'abc';
    $data = ['PS1' => 'test1', 'PS2' => 13.1415, 'PS3' => 'test2'];
    ...
    $littleDesc = ['Name' => 'sometext', 'IO' => I5_INOUT, 'Type' => I5_TYPE_CHAR, 'Length' => 20];
...
if ($doPgmCallSimple) {
    ...
    $desc = [
        ['Name' => 'code', 'IO' => I5_INOUT, 'Type' => I5_TYPE_CHAR, 'Length' => '10'],
        ['Name' => 'name', 'IO' => I5_INOUT, 'Type' => I5_TYPE_CHAR, 'Length' => '10'],
    ];
    ...
    /* Execute Program */
    $params = ['code' => '123', 'name' => 'ABC'];
    $retvals = ['code' => 'code', 'name' => 'name'];
    ...
    if ($close_val === false) {
        echo "FAIL : i5_program_close returned fales, closing an open prog.<br>\n";
        $errorTab = i5_error();
        var_dump($errorTab);
    }
} // (simple call)
...
$start = microtime(true);
$commandSuccessful = i5_command($cmdString, [], [], $conn);
$end = microtime(true);
$elapsed = $end - $start;
...
$conn->setToolkitServiceParams(['plugSize' => '5M']); // bigger to handle large joblog
$interactiveOutput = $conn->CLInteractiveCommand($cmdString);
$conn->setToolkitServiceParams(['plugSize' => '512K']); // put back to default
echo printArray($interactiveOutput) . '<BR><BR>';

$msg = 'HELLO_WITH_INPUTS_ARRAY';
$cmdString = 'SNDMSG';
$inputs = ['MSG' => $msg, 'TOUSR' => $user];
...
if ($doJobLists) {
    ...
    $list = i5_job_list([I5_JOBNAME => 'ZENDSVR']);
    if (!$list) {
...
if ($doSpooledFiles) {
    ...
    $list = i5_spool_list([I5_USERNAME => $splUser], $conn);
    if (!$list) {
    ...
    $list = i5_spool_list([I5_OUTQ => $outq], $conn);
    if (!$list) {
        echo 'Error getting spool list: ' . printArray(i5_error()) . '<BR>';
    } else {
        $spoolCount = 0;
