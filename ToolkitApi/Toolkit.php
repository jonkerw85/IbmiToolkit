...
class Toolkit implements ToolkitInterface
{
    ...
    */
    ...
    /**
     * @var resource|PDO|PdoSupp|odbcsupp|db2supp|null
     */
    ...
    protected $_validSeparators = ['.', '/'];

    protected $_outputVarsToExport = [];
    ...
    protected $_cpfMapping = ['QSYRUSRI' => ['QLIROHDL'], 'QSYGETPH' => ['QSYPHDL']];
    ...
    public function __construct($databaseNameOrResource, $userOrI5NamingFlag = '0', $password = '', $transportType = '', $options = ['isPersistent' => false])
    {
        ...
        $this->getOptionalParams('system', ['v5r4', 'ccsidBefore', 'ccsidAfter', 'useHex', 'paseCcsid', 'trace', 'dataStructureIntegrity',  'arrayIntegrity']);
        $this->getOptionalParams('transport', ['httpTransportUrl', 'plugSize', 'xmlserviceCliPath']);

        // populate serviceParams with $transport, or get it from INI
        if (!$transportType) {
            $this->getOptionalParams('transport', ['transportType']);
        } else {
        ...
        if ($transportType === 'ibm_db2' && $isResource) {
            ...
            $this->setOptions(['schemaSep' => $schemaSep]);
            ...
            $this->chooseTransport($transportType);
            $transport = $this->getTransport();
            $conn = $transport->connect($databaseName, $user, $password, ['persistent' => $this->getIsPersistent()]);
        } else {
            $databaseName = $databaseNameOrResource;
            $user = $userOrI5NamingFlag;
    ...
    protected function setDb($transportType = '')
    {
        ...
        if ($extensionName === 'ibm_db2') {
            $this->setOptions(['plugPrefix' => 'iPLUG']);
            $this->db = new db2supp();
            ...
            //for odbc will be different default stored procedure call
                $this->setOptions(['plugPrefix' => 'iPLUGR']); // "R" = "result set" which is how ODBC driver returns param results
                $this->db = new odbcsupp();
        } elseif ($extensionName === 'pdo') {
            $this->setOptions(['plugPrefix' => 'iPLUGR']);
    ...
    {
        $this->setOptions([$callType => true]);
        $outputArray = $this->PgmCall('NONE', null, null, null);
        $this->setOptions([$callType => false]);
    ...
    public function pgmCall($pgmName, $lib, $inputParam = null, $returnParam = null, $options = null)
    {
        ...
        $this->XMLWrapper = new XMLWrapper(['encoding' => $this->getOption('encoding')], $this);
        ...
        if ($outputXml != '') {
            ...
            if (!is_array($outputParamArray)) {
                ...
                $programsToLookFor = [$pgmName, '< lveContext', '#mnrnrl', 'QRNXIE', '< allProgram'];
    ...
    public function ExecuteProgram($inputXml, $disconnect = false)
    {
        ...
        if (isset($this->db) && $this->db) {
        ...
        } elseif ($this->getTransport() instanceof LocalSupp) {
        ...
        } elseif ($this->getTransport() instanceof SshSupp) {
            $transport = $this->getTransport();
            $xmlserviceCliPath = $this->getOption('xmlserviceCliPath');
            $transport->setXmlserviceCliPath($xmlserviceCliPath);
            ...
            $result = $transport->send($inputXml);
        } elseif ($this->getTransport() instanceof httpsupp) {
            $transport = $this->getTransport();
    ...
    {
        return new XMLWrapper(['encoding' => $this->getOption('encoding')], $this);
    }
...
//        $successFlag = $this->XMLWrapper->getCmdResultFromXml($outputXml, $parentTag);
    ...
    public function CLCommand($command, $exec = '')
    {
        ...
        $expectDataOutput = in_array($exec, ['rexx', 'pase', 'pasecmd']);
    ...
    */
    ...
    {
        $this->setOptions(['internalKey' => $internalKey]);
    }
    ...
    protected function extractErrorFromJoblog(array $programsToLookFor)
    {
        ...
        if ($startingLine) {
             ...
             */
            $old = [$messageLabel, $causeLabel, $recoveryLabel, '   ', '  '];
            $new = ['', '', '', ' ', ' '];
